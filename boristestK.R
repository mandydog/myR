#x <- rep(1, times=100)
y <- c(  rep(0, times=1), rep(20, times=49), rep(21, times=50) )

#dataframe <- data.frame (x, y)
#means <- colMeans(dataframe) #this is a vector


#rangemark <- max(dataframe[,"mark"]) - min(dataframe[,"mark"])
rangey <- max(y)- min(y) 
#rangeoccupation <- max(dataframe[,"occupation"]) - min(dataframe[,"occupation"]) 

newy <- (y- mean(y) )/ rangey

#dataframe2 <- cbind(  (dataframe[,"mark"] - mean(mark)) / rangemark,
#                      (dataframe[,"occupation"] - mean(occupation))/ rangeoccupation   )
                     

#fit <- kmeans(dataframe, 2, iter.max=900, nstart=1 )
fit <- kmeans(newy, 2, iter.max=900, nstart=3 )
print(fit)
#fit <- kmeans(mydata, 3) # 3 cluster solution
#kmeans(x, centers, iter.max = 10, nstart = 1, algorithm = c("Hartigan-Wong", "Lloyd", "Forgy", "MacQueen"))
#kmeans(x, centers, iter.max = 10, nstart = 1, algorithm = c("Hartigan-Wong", "Lloyd", "Forgy", "MacQueen"))

plot(newy, col = fit$cluster)
# plot centers
points(fit$centers, col = 1:2, pch = 8)
